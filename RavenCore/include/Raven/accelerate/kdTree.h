 ...
 public:
  ...
  void createLeaf(int n, const int* prims, std::vector<int>* primIndices) {
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; i++) { primIndices->push_back(prims[i]); }
    }
  }
  ...
  // data
  union {
    float splitPos;  // if this node is interior node, record the split position
                     // along the axis
    int onePrimitive;  // if this node is leaf node and only has one primitive
                       // inside, recode the index of primitive
    int indexOffset;   // if this node is leaf node and has several primitives
                       // inside, record the offset of first index inside
                       // primitiveIndices array
  };
  union {
    int flag;  // the lower two bits indicate whether this node is leaf node or
               // record the split axis
    int nPrims;      // if this node is leaf node, the upper 30 bits record the
                     // number of primitives within this node
    int aboveChild;  // if this node is interior node, the upper 30 bits record
                     // the index of above child of this node
  };
};

class KdTreeAccel final : public Accelerate {
 private:
  void buildNode(int*                         nodeNum,
                 int                          depth,
                 const int*                   pIndStart,
                 int                          nPrimitives,
                 const Bound3f&               nodeBounds,
                 const std::vector<Bound3f>&  allPrimBounds,
