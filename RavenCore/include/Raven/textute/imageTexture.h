...
struct TextureInfo {
  ...
  TextureInfo(const std::string& path,
              ...
              ImageWrap          wrap,
              bool               gamma)
      : path(path), doTrilinear(doTrilinear), wrap(wrap), gamma(gamma) {}
  bool operator<(const TextureInfo& t) const {
    if (path != t.path)
      return path < t.path;
    if (doTrilinear != t.doTrilinear)
      return doTrilinear < t.doTrilinear;
    if (gamma != t.gamma)
      return !gamma;
    return wrap < t.wrap;
  }
...
class ImageTexture final : public Texture<TMemory> {
 ...
 private:
  ...
  static Mipmap<TMemory>* getTexture(const std::string& path,
                                     bool               doTrilinear,
                                     ImageWrap          wrap,
                                     bool               gamma);

  static void convertIn(const RGBSpectrum& from,
                        double&            to,
                        bool               gamma = true) {
  ...
  static void convertOut(double from, double& to) { to = from; }
...
Ref<ImageTexture<RGBSpectrum, Spectrum>> makeImageTextureSpectrum(
    const PropertyList& param);
}  // namespace TextureBuild
