...
/// <summary>
/// Triangle mesh classes stores all infomation about triangles inside and hold
/// the instances of relative triangle array
/// </summary>
class TriangleMesh : public RavenObject {
 public:
  TriangleMesh(const Ref<Transform>&        OTW,
               const Ref<Transform>&        WTO,
               int                          triNum,
               const std::vector<Point3f>&  vs,
               const std::vector<int>&      ins,
               const std::vector<Normal3f>& ns,
 ...
 public:
  ...
  const int nVertices;
  std::vector<Point3f>
      vertices;  // store all vertices in one array to save memory
  std::vector<Normal3f> normals;
  std::vector<Vector3f> tangants;  // alternative
  std::vector<Point2f>  uvs;       // alternative

  std::vector<int> indices;  // indices of triangle indices of ith triangle are
                             // indices[3*i] to indices[3*i+2]
...
class Triangle : public Shape {
 public:
  Triangle(const Ref<Transform>& LTW,
  ...
  Point3f getVertex(int num) const { return mesh->vertices[index(i)]; }
...
std::shared_ptr<TriangleMesh> CreatePlane(const Ref<Transform>& LTW,
                                          const Ref<Transform>& WTL,
                                          const Point3f&        v0,
                                          const Point3f&        v1,
                                          const Point3f&        v2,
