...
namespace Raven {
using Float           = double;
static Float MaxFloat = std::numeric_limits<double>::max();
template <typename T>
...
inline double Lerp(double t, double x1, double x2) {
  return (1 - t) * x1 + t * x2;
}
...
inline bool Quadratic(double  a,
                      double  b,
                      double  c,
                      double& tmin,
                      double& tmax) {
...
inline bool    IsPowerOf2(int32_t x) { return x && !(x & (x - 1)); }
...
inline bool solve2x2LinearSystem(const double* A,
                                 const double* b,
                                 double*       x0,
                                 double*       x1) {
...
inline double rgbToFloat(int rgb_value) { return rgb_value / 255.0; }
...
class RavenParamSetItem {
  RavenParamSetItem(const std::string&          name,
