...
inline Vector3f reflect(const Vector3f& wo,
                        const Normal3f& n = Normal3f(0, 0, 1)) {
...
inline std::optional<Vector3f> Refract(const Vector3f& wo,
                                       const Vector3f& n,
                                       double          eta) {
...
class FresnelConductor : public Fresnel {
 ...
 public:
  FresnelConductor(double etai, double etat, double k)
...
class BxDF {
 public:
  ...
  virtual Spectrum sampled_f(const Vector3f& wo,
                             Vector3f&       wi,
                             const Point2f&  sample,
                             double&         pdf) const = 0;
  virtual Spectrum hdf(const Vector3f& wo, int nSamples, Point2f* samples) {
    return Spectrum(0.0);
  }
  virtual Spectrum hhf(int nSamples, Point2f* outSamples, Point2f* inSamples) {
    return Spectrum(0.0);
  }
...
class scaledBxDF : public BxDF {
 public:
  scaledBxDF(BxDFType type, BxDF* bxdf, Spectrum scaler)
      : BxDF(type), bxdf(bxdf), scaler(scaler) {}
...
inline double Cos2Theta(const Vector3f& w) { return w.z * w.z; }

inline double AbsCosTheta(const Vector3f& w) { return abs(w.z); }

inline double Sin2Theta(const Vector3f& w) {
  return Max(0.0, 1. - Cos2Theta(w));
}

inline double SinTheta(const Vector3f& w) { return sqrt(Sin2Theta(w)); }

inline double TanTheta(const Vector3f& w) { return SinTheta(w) / CosTheta(w); }

inline double AbsTanTheta(const Vector3f& w) {
  return abs(SinTheta(w) / CosTheta(w));
}

inline double Tan2Theta(const Vector3f& w) { return TanTheta(w) * TanTheta(w); }
...
inline double Cos2Phi(const Vector3f& w) { return CosPhi(w) * CosPhi(w); }

inline double Sin2Phi(const Vector3f& w) { return SinPhi(w) * SinPhi(w); }
