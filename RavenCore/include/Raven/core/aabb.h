...
class AABB3 {
 public:
  ...
  AABB3<T>(const AABB3<T>& box) {
    this->pMax = box.pMax;
    this->pMin = box.pMin;
  }

  AABB3<T>(const Point3<T>& p) {
    pMax = p;
    pMin = p;
  }
  ...
  Point3<T> corner(int corner) const {
    return Point3<T>((*this)[corner & 1].x, (*this)[corner & 2].y,
                     (*this)[corner & 4].z);
  }

  Vector3<T> diagoal() const { return pMax - pMin; }
  ...
  Point3<T> lerp(const Point3<T>& t) const {
    return Point3<T>(Lerp(t.x, pMax.x, pMin.x), Lerp(t.y, pMax.y, pMin.y),
...
class AABB2 {
 public:
  ...
  Point2<T> Lerp(const Point3<T>& t) const {
    return Point2<T>(Lerp(t.x, pMax.x, pMin.x), Lerp(t.y, pMax.y, pMin.y));
  }
};

template <class T>
inline AABB3<T> Union(const AABB3<T>& box1, const AABB3<T>& box2) {
  return AABB3<T>(Point3<T>(box1[0].x < box2[0].x ? box1[0].x : box2[0].x,
...
inline AABB3<T> Union(const AABB3<T>& box, const Point3<T>& p) {
  return AABB3<T>(Point3<T>(Min(box.pMin.x, p.x), Min(box.pMin.y, p.y),
                            Min(box.pMin.z, p.z)),
                  Point3<T>(Max(box.pMax.x, p.x), Max(box.pMax.y, p.y),
                            Max(box.pMax.z, p.z)));
}
