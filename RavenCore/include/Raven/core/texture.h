...
class TextureMapping3D : public RavenObject {
 public:
  virtual std::tuple<Point3f, Vector3f, Vector3f> map(
      const SurfaceInteraction& si) const = 0;
};
...
class ConstTexture : public Texture<T> {
 public:
  ...
  virtual T evaluate(const SurfaceInteraction& its) const override {
    return value;
  }
...
class ScaleTexture : public Texture<T> {
 public:
  ...
  ScaleTexture(const Ref<Texture<T>>& t1, const Ref<Texture<T>>& t2)
      : vTexture(t1), sTexture(t2) {}
...
Ref<Texture<Spectrum>> makeConstTextureSpectrum(const PropertyList& param);
   ...
   //
   //	_RAVEN_CLASS_REG_(constfloat, ConstTextureFloat,
   // ConstTexture<double>::construct)
   //
   //		_RAVEN_CLASS_REG_(constspectra, ConstTextureSpectra,
   // ConstTexture<Spectrum>::construct)
   //
   //		//_RAVEN_CLASS_REG_(scalefloat, ScaleTextureFloat,
   // TextureBuild::makeScaleTextureFloat)
   //
   //		//_RAVEN_CLASS_REG_(scalespectra, ScaleTextureSpectra,
   // TextureBuild::makeScaleTextureSpectrum)
   //}
}  // namespace Raven
