...
// Brife Description :
// Apart from the Sampler classes,UniformSample uniformly maps the random
// numbers of [0,1) generated by Sampler to domains where Monte Carlo estimaters
// will be processed.
...
// return a random point in a unit disk generated with respect to given uv
// parameter
...
inline double UniformDiskPdf() { return 1 / M_PI; }
...
inline double UniformSpherePdf() { return 0.25 / M_PI; }
...
inline double UniformHemispherePdf() { return 0.5 / M_PI; }
...
inline double UniformTrianglePdf(double area) { return 1.0 / area; }
...
struct Distribution1D {
 public:
  // 输入一组分段常值函数
  Distribution1D(const double* f, int n) : function(f, f + n), cdf(n + 1) {
    double invN = 1.0 / n;
    integral    = 0;
    // 假设定义域为0-1，使用黎曼积分计算定积分的值
    for (int i = 0; i < n; i++) integral += function[i] * invN;

    // 计算CDF，CDF为PDF的积分，PDF等于函数在某个区间的积分除以函数在定义域上的积分
    cdf[0] = 0;
    if (integral == 0) {
      for (int i = 1; i < n + 1; i++) cdf[i] = double(i) / n;
    } else {
  ...
  int sampleDiscrete(double u, double* pdf, double* remappedU = nullptr) const {
    ...
    if (remappedU)
      *remappedU =
          (u - function[offset]) / (function[offset + 1] - function[offset]);
    return offset;
  }
...
class Distribution2D {
 public:
  Distribution2D(const double* data, int nu, int nv) {
    ...
    for (int i = 0; i < nv; i++) integralU[i] = conditionalU[i]->funInte();
  ...
  double pdf(const Point2f& uv) const {
    int iu = (int)Clamp(int(uv[0] * conditionalU[0]->size()), 0,
                        conditionalU[0]->size() - 1);
    int iv =
        (int)Clamp(int(uv[1] * marginalV->size()), 0, marginalV->size() - 1);
