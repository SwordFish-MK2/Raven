...
class LambertainReflection : public BxDF {
 public:
  LambertainReflection(Spectrum albedo)
      : BxDF(BxDFType(Reflection | Diffuse)), albedo(albedo) {}
  ...
  Spectrum hdf(const Vector3f& wo, int nSamples, Point2f* samples) {
    return albedo;
  }

  Spectrum hhf(int nSamples, Point2f* outSamples, Point2f* inSamples) {
    return albedo;
  }
