...
class SpecularReflection final : public BxDF {
 public:
  ...
  Spectrum sampled_f(const Vector3f& wo,
                     Vector3f&       wi,
                     const Point2f&  sample,
                     double&         pdf) const;

  double pdf(const Vector3f& wo, const Vector3f& wi) const { return 0.0; }
...
class SpecularTransmission final : public BxDF {
 public:
  SpecularTransmission(const Spectrum& t, double etaI, double etaT)
      : BxDF(BxDFType(Specular | Transmission)),
        t(t),
        fresnel(etaI, etaT),
        etaA(etaI),
        etaB(etaT) {}
...
class FresnelSpecular final : public BxDF {
 public:
  FresnelSpecular(const Spectrum& r,
                  const Spectrum& t,
                  double          etaa,
                  double          etab)
  ...
  double pdf(const Vector3f& wo, const Vector3f& wi) const override {
    return 0.0;
  }
