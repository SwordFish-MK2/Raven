...
class BeckmannSpizzichino : public MicrofacetDistribution {
 ...
 public:
  BeckmannSpizzichino(double alphaX, double alphaY, bool sampleVNDF = false)
      : alphaX(Max(0.001, alphaX)),
        alphaY(Max(0.001, alphaY)),
        sampleVNDF(sampleVNDF) {}
...
class MicrofacetReflection : public BxDF {
 ...
 public:
  MicrofacetReflection(std::shared_ptr<Fresnel>                fresnel,
                       std::shared_ptr<MicrofacetDistribution> mircrofacet,
                       Spectrum                                albedo);
...
//	double etaA, etaB;
// public:
//	MicrofacetTransmission(std::shared_ptr<Fresnel> fresnel,
//std::shared_ptr<MicrofacetDistribution> microfacet, 		double etaA, double etaB,
//Vector3f albedo); 	virtual Vector3f f(const Vector3f& wo, const Vector3f&
//wi)const; 	virtual Vector3f sampled_f(const Vector3f& wo, Vector3f& wi, const
//Point2f& uv, double* pdf)const {
...
std::tuple<double, double> BeckmannSample11(const Vector3f& wo,
                                            const Point2f&  uv);
}  // namespace Raven
