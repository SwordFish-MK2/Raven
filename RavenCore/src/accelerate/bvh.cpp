...
BVHAccel::BVHAccel(const std::vector<std::shared_ptr<Primitive>>& primitives,
  ...
  std::shared_ptr<BVHNode> root =
      recursiveBuild(primInfo, 0, primInfo.size(), ordered, totalNodes);
  ...
  std::cout << "BVH generation complete, total node number:" << totalNodes
            << std::endl;
}
...
std::shared_ptr<BVHNode> BVHAccel::recursiveBuild(
  ...
  // 求当前node的boundingBox
  Bound3f bounds;
  for (size_t i = start; i < end; i++) bounds = Union(bounds, info[i].box);
  ...
  if (nPrimitive == 1) {
    ...
    // 有多个图元且中心点不重合，尝试划分
      ...
      if (nPrimitive <= 2) {
        ...
        middle = (start + end) / 2;
        std::nth_element(
            &info[start], &info[middle], &info[end - 1] + 1,
        ...
        for (int i = 0; i < nBuckets - 1; i++) {
          ...
          cost[i] = 0.125 + (Max(0.0, left.surfaceArea()) * leftCount +
        ...
        if (nPrimitive > maxPrimInNode || minCost < leafCost) {
          PrimitiveInfo* pmid = std::partition(
              &info[start], &info[end - 1] + 1, [=](const PrimitiveInfo& pi) {
                int b = nBuckets * centroidBound.offset(pi.centroid)[axis];
                if (b == nBuckets)
                  b = nBuckets - 1;
                assert(b >= 0);
                assert(b < nBuckets);
      ...
      std::shared_ptr<BVHNode> leftNode =
          recursiveBuild(info, start, middle, ordered, totalNodes, depth);
      std::shared_ptr<BVHNode> rightNode =
          recursiveBuild(info, middle, end, ordered, totalNodes, depth);
    ...
    if (node->box.hit(ray, &t0, &t1)) {
      ...
      if (node->nPrims > 0) {
        ...
        for (size_t i = 0; i < node->nPrims; i++) {
          // 与节点内的Primitive求交
          size_t index             = node->firstOffset + i;
          bool   foundIntersection = prims[index]->intersect(ray, record);
