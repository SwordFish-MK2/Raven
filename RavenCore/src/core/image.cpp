...
Image<RGBSpectrum> ReadImage(const std::string& path) {
  ...
  std::unique_ptr<Spectrum[]> convertedData =
      std::make_unique<Spectrum[]>(w * h);
  if (c == 3) {
    for (int i = 0; i < w * h; i++) convertedData[i] = toSpectrum(&data[i * 3]);
  } else if (c == 4) {
    for (int i = 0; i < w * h; i++) {
...
void WriteImage(const Image<RGBSpectrum>& image, const std::string& filename) {
  unsigned char* colorData = (unsigned char*)malloc(
      sizeof(unsigned char) * 3 * image.uSize() * image.vSize());
  ...
  stbi_write_jpg(filename.c_str(), image.uSize(), image.vSize(), 3, colorData,
                 0);
  free(colorData);
}
