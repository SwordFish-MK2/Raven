...
SurfaceInteraction Sphere::getGeoInfo(const Point3f& pHit) const {
  ...
  Vector3f dpdv =
      M_PI * Vector3f(pHit.y * cosPhi, -sin(theta) * radius, -pHit.y * sinPhi);
  ...
  Vector3f dndu = (f * F - e * G) * dpdu * temp + (e * F - f * E) * dpdv * temp;
  Vector3f dndv = (g * F - f * G) * dpdu * temp + (f * F - g * E) * dpdv * temp;
...
std::tuple<SurfaceInteraction, double> Sphere::sample(const Interaction& inter,
  ...
  if (distanceSquared < radius * radius) {
    ...
    if (dis2 == 0.0)
      ...
      wi = Normalize(wi);
      pdf *=
          DistanceSquared(lightInter.p, inter.p) / abs(Dot(lightInter.n, -wi));
    }
    if (std::isinf(pdf))
      pdf = 0.0;
  ...
  // 采样p点可见圆锥
    ...
    // 计算对于pL对于圆心的角度alpha
    double ds =
        dc * cosTheta - std::sqrt(Max(0.0, radius * radius - dc * sinTheta2));
    double cosAlpha = (dc * dc + radius * radius - ds * ds) / (2 * dc * radius);
    double sinAlpha = std::sqrt(Max(0.0, 1 - cosTheta * cosTheta));

    Vector3f nWorld = sinAlpha * cos(phi) * (-x) + sinAlpha * sin(phi) * (-y) +
                      cosAlpha * (-z);
