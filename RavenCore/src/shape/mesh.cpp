...
namespace Raven {
TriangleMesh::TriangleMesh(const Ref<Transform>&        OTW,
...
Bound3f Triangle::localBound() const {
  ...
  p1         = (*worldToLocal)(p1);
  p2         = (*worldToLocal)(p2);
  box.pMin   = Point3f(Min(p0.x, p1.x, p2.x), Min(p0.y, p1.y, p2.y),
                       Min(p0.z, p1.z, p2.z));
  box.pMax   = Point3f(Max(p0.x, p1.x, p2.x), Max(p0.y, p1.y, p2.y),
                       Max(p0.z, p1.z, p2.z));
  return box;
}
...
std::tuple<SurfaceInteraction, double> Triangle::sample(
...
std::shared_ptr<TriangleMesh> CreatePlane(const Ref<Transform>& LTW,
  ...
  std::vector<Point2f>  uvs      = {Point2f(0, 1), Point2f(1, 1), Point2f(1, 0),
                                    Point2f(0, 0)};
  std::vector<Normal3f> normals{normal, normal, normal, normal};
  std::vector<Vector3f> tangants;
  std::shared_ptr<TriangleMesh> mesh = std::make_shared<TriangleMesh>(
      LTW, WTL, 2, vertices, indices, normals, tangants, uvs);
  return mesh;
}
...
//   const Ref<Transform>& otw =
//       std::dynamic_pointer_cast<Transform>(otwRef.getRef());
