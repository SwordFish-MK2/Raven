...
namespace Raven {
	Spectrum DiffuseAreaLight::Li(
		const SurfaceInteraction& p, 
	...
	Spectrum DiffuseAreaLight::sampleLi(
		const SurfaceInteraction& inter,
		 const Point2f& uv,
		LightSample* lightSample)const
	{
		//sample a random point on light surface, compute light direction from this point to given point p
	...
	double DiffuseAreaLight::pdf_Li(
		const SurfaceInteraction& inter,
		 const Vector3f& wi)const 
	{
