...
namespace Raven {
	InfiniteAreaLight::InfiniteAreaLight(
		...
		for (int v = 0; v < height; v++) {
			...
			double sinTheta = std::sin(M_PI * double(v + 0.5) / double(height));//生成sintheta用于消除扭曲
			for (int u = 0; u < width; u++) {
				double up = double(u) / double(width);
				img[u + v * width] = lightMap->lookup(Point2f(up, vp), filter).y();//使用采样到的Spectrum的亮度来生成pdf
	...
	Spectrum InfiniteAreaLight::sampleLi(const SurfaceInteraction& inter, const Point2f& rand, LightSample* lightSample)const {
		double pdf = 0;
		Point2f uv = distribution->sampleContinuous(rand, &pdf);//根据光源强度分布生成样本
