...
std::optional<TriangleInfo> Loader::load(const std::string& path,
  ...
  bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warning, &error,
                              file_name, mtl_base_path, true);
  ...
  for (size_t s = 0; s < shapes.size(); s++) {
    ...
    for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
      const int verticesNum =
          shapes[s].mesh.num_face_vertices[f];  // 这个面有多少个顶点
      for (int i = 0; i < verticesNum; i++) {
        tinyobj::index_t idx =
            shapes[s].mesh.indices[indexOffset + i];  // 该面第i个顶点的索引信息
  ...
  std::cout << "Successfully loaded file: " << path
            << ", triangle number: " << faceNum
...
// std::unique_ptr<Vector3f[]> ReadImageVector(const std::string& path, Point2i*
// resolution) { 	const char* filename = (char*)path.c_str(); 	int w, h, c;
//	unsigned char* data = stbi_load(filename, &w, &h, &c, 0);
//	*resolution = Point2i(w, h);
//	std::unique_ptr<Vector3f[]> convertedData =
//std::make_unique<Vector3f[]>(w * h);
//	//TODO::未读取到图片
//	//TODO::判断图片的通道数
//	//如果图片为三通道
//	for (int i = 0; i < w * h; i++) {
