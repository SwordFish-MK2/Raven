...
bool Parser::parse(const std::string& file) {
          ...
          [&](const tinyxml2::XMLElement* node, PropertyList* pList,
            ...
            if (isScene) {
              ...
              if (accelerationAttribute) {
                std::string accType =
                    node->FindAttribute("accelerate")->Value();
                if (accType == "bvh") {
              ...
              switch (tag) {
                ...
                case RFilm: {
                  ...
                  // 成功获取幕布类型
                    ...
                    // 在xml中指定的对象类型未在相机工厂中注册（即指定了raven不支持的对象
                    if (factory.registed(type) == false) {
                      std::cout << "!!!!!!!!!!!!!!!!!!!!!";
                      return;
                    }
                ...
                case RCamera: {
                  ...
                  const tinyxml2::XMLAttribute* typeAttr =
                      node->FindAttribute("type");
                  if (!typeAttr) {
                    std::cout << "No Camera type is specified,Raven abort.\n";
                    return;
                  }
                  ...
                  // 成功获取幕布类型
                    ...
                    // 在xml中指定的相机类型未在相机工厂中注册（即指定了raven不支持的相机
                    if (factory.registed(type) == false) {
                      std::cout << "!!!!!!!!!!!!!!!!!!!!!";
                      return;
                    }
                  ...
                  case RMaterial: {
                    ...
                    if (!idAttr)
                      ...
                      PropertyList::setObjectRefById(type, id, my_object,
                                                     *pList);
                    }
                    break;
                  }
                  ...
                  case REmit: {
                    ...
                    std::tuple<std::string, RavenLightType, PropertyList>
                        lProperty = std::make_tuple(id, lightIt->second, list);
                ...
                switch (tag) {
                  ...
                  case RSpectra: {
                    ...
                    if (type == "rgb") {
                      ...
                      Spectrum sampledSpectra = Spectrum::fromSampled(
                          &lambda[0], &spectrumValue[0], n);
                  ...
                  case RPointer: {
                    ...
                    pointerMap["transform"] =
                        RavenPointerType::transform_pointer;
                    ...
                    pointerMap["primitive"] =
                        RavenPointerType::primitive_pointer;
  ...
  std::shared_ptr<RavenRenderingPrograme>
  Parser::makePrograme() const {
    ...
    for (size_t i = 0; i < textureProperty.size(); i++) {
      ...
      if (tt == RavenTextureType::RConstFloat) {
        ...
        spectrumTexture[id] = makeCheckeredSpectrum(
            oddIt->second, evenIt->second, mappingIt->second, ppt);
